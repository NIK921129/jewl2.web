/* =========================================================
   NOVA STORE — runtime config (merged here; single script)
   API_BASE_URL: keep "/api" when the host proxies /api to the
   backend (Vercel/Render). Use "https://your-api.onrender.com/api"
   for direct cross-origin calls.
   ========================================================= */
window.NOVA_CONFIG = {
  API_BASE_URL: "/api",
  SOCKET_URL: "",
...
async function api(path, { method = "GET", body, raw = false } = {}) {
  ...
  if (!res.ok) {
    // A static host without the /api proxy answers 404 — fall back to demo data for reads.
    if (CFG.ALLOW_DEMO_FALLBACK && method === "GET") { DEMO = true; return demoApi(path, method, body); }
