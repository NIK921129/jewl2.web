...
const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, process.env.UPLOADS_DIR || path.join(__dirname, "uploads")),
...
function rateLimit({ windowMs = 60_000, max = 20, key = "generic" } = {}) {
  return (req, res, next) => {
...
setInterval(() => {
  const now = Date.now();
  for (const [id, hit] of buckets) if (now > hit.reset) buckets.delete(id);
}, 300_000).unref?.();
...
router.get(
  ...
  auth(true),
  wrap(async (req, res) => {
    if (req.user.id === "admin") return res.json({ name: "Administrator", email: ADMIN_ID, role: "admin" });
...
router.put(
  ...
  auth(true),
  wrap(async (req, res) => {
    if (req.user.id === "admin") return res.status(400).json({ error: "Administrator profile cannot be edited here" });
...
router.post(
  ...
  auth(true),
  wrap(async (req, res) => {
    if (req.user.id === "admin") return res.status(400).json({ error: "Change ADMIN_PASSWORD in your environment variables" });
